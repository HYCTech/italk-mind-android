...
public class NettyClient {
    ...
    public synchronized NettyClient connect() {
        if (!isConnect) {
            ...
            try {
                ...
                ChannelFuture future = bootstrap.connect(LWUserManager.getInstance().getLinkServerInfo().getMajorLinkIp() ,LWUserManager.getInstance().getLinkServerInfo().getMajorLinkPort()).sync();
     ...
     */
    public void sendMessage(ItalkMindMessage vo, FutureListener futureListener) {
...
//        final String s = gson.toJson(vo);
        if (futureListener == null) {
            channel.writeAndFlush(vo).addListener(new FutureListener() {
                ...
                @Override
                public void success() {
                    Log.e(TAG, "发送成功--->" + vo.toString());
                }

                @Override
                public void error() {
                    Log.e(TAG, "发送失败--->" + vo.toString());
                }
            });
        } else {
