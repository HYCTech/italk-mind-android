...
import com.lw.italk.http.model.LoginRequest;
...
public class TelLoginActivity extends BaseActivity implements Response{
    @BindView(R.id.logo)
    TextView logo;
    ...
    private void changeLoginMode() {
        if (mLoginMode == LOGIN_MODE_VERIFICATION_CODE) {
            mTvForgetPswd.setVisibility(View.GONE);
            line3.setVisibility(View.GONE);
            mTvChangeMode.setText("用密码登录");
            tvVerificationCode.setText("验证码");
            mSendVerificationCode.setVisibility(View.VISIBLE);
        } else {
            mTvForgetPswd.setVisibility(View.VISIBLE);
            line3.setVisibility(View.VISIBLE);
            mTvChangeMode.setText("用验证码登录");
            tvVerificationCode.setText("密码");
            mSendVerificationCode.setVisibility(View.GONE);
        }
    }
    ...
    @OnClick({R.id.phone_number, R.id.send_verification_code, R.id.verification_code, R.id.login, R.id.tv_forget_pswd, R.id.tv_account_login, R.id.tv_change_mode})
...
//                    HttpUtils.doPost(this, Request.Path.USER_LOGINSMS, request, true, Request.Code.USER_LOGINSMS, this);
            ...
            case R.id.tv_forget_pswd:
                Intent intent1 = new Intent(TelLoginActivity.this, TelVerificationActivity.class);
                startActivity(intent1);
                break;
            case R.id.tv_account_login:
                Intent intent = new Intent(TelLoginActivity.this, ATAccountRegisterActivity.class);
                startActivity(intent);
                finish();
                break;
            case R.id.tv_change_mode:
                mLoginMode = mLoginMode == LOGIN_MODE_VERIFICATION_CODE ? LOGIN_MODE_PSWD : LOGIN_MODE_VERIFICATION_CODE;
                changeLoginMode();
                break;
            case R.id.login:
        ...
        switch (requestCode) {
            ...
            case Request.Code.USER_ALLPROFILE:
                ...
                if(!TextUtils.isEmpty(((UserInfo) o).getUsername())){
                    ...
                    Intent intent = new Intent(TelLoginActivity.this, PerfectInformationActivity.class);
                    startActivity(intent);
                    finish();
                }
                break;
