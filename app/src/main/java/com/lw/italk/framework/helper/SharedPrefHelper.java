...
public class SharedPrefHelper {
    ...
    public void setLinkInfo(LoginInfo.LinkServerInfo linkInfo) {
        sharedPreferences.edit().putString("majorLinkIp", linkInfo.getMajorLinkIp()).commit();
        sharedPreferences.edit().putInt("majorLinkPort", linkInfo.getMajorLinkPort()).commit();
        sharedPreferences.edit().putString("secondaryLinkIp", linkInfo.getSecondaryLinkIp()).commit();
        sharedPreferences.edit().putInt("secondaryPort", linkInfo.getSecondaryPort()).commit();
        sharedPreferences.edit().putString("authId", linkInfo.getAuthId()).commit();
        sharedPreferences.edit().apply();
    }
    public LoginInfo.LinkServerInfo getLinkInfo(){
        LoginInfo.LinkServerInfo linkInfo = new LoginInfo.LinkServerInfo();
        linkInfo.setMajorLinkIp(sharedPreferences.getString("majorLinkIp",""));
        linkInfo.setMajorLinkPort(sharedPreferences.getInt("majorLinkPort",0));
        linkInfo.setSecondaryLinkIp(sharedPreferences.getString("secondaryLinkIp",""));
        linkInfo.setSecondaryPort(sharedPreferences.getInt("secondaryPort",0));
        linkInfo.setAuthId(sharedPreferences.getString("authId",""));

        if (linkInfo.getMajorLinkIp().length() == 0){
            return  null;
        }
        return linkInfo;
    }
    ...
    public void setFileAuthId(String fileAuthId) {
        sharedPreferences.edit().putString("fileAuthId", fileAuthId).commit();
        sharedPreferences.edit().apply();
    }
    public String getFileAuthId() {
        return sharedPreferences.getString("fileAuthId","");
    }
    ...
    public String getTokenId() {
        return sharedPreferences.getString("tokenId",null);
    }
}
